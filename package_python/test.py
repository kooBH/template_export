...
input = rs.load(cur_path+ "/../input/input.wav", mono=False, sr = 16000)[0]
...
# Linux
    ...
    path_lib = cur_path + "/ALGO.so"
     ...
     exit(-1)
processor = ALGO(path_lib)

n_frame = int(input.shape[1]/n_hop)

interleaved_in = np.zeros(n_channels*input.shape[1])
...
sf.write(cur_path + "/../output/test_output.wav",data_out,16000)